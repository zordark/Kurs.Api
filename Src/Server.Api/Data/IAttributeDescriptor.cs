...
namespace Kurs.Api.Server.Data
{
    public interface IAttributeDescriptor
    {
        ...
        /// <summary>
        /// Ссылка на мета-тип, который является владельцем атрибута.
        /// Если, атрибут глобальный, то владельцем атрибута будет глобальный дескриптор типа
        /// </summary>
        ITypeDescriptor OwnerTypeDescriptor { get; }

        /// <summary>
        /// Используется при описании аргументов команд и событий.
        /// Задает ссылку на описатель объекта адаптера.
        /// То есть при заданиии значения для этого аргумента нельзя задать произвольное значение, а необходимо указать объект
        /// </summary>
        ITypeDescriptor RefToObject { get; }
